    ...
    <div class="container">
      ...
      <div class="form">
        ...
        <div class="form-row">
          ...
          <!-- 国家 -->
          <div class="field field--sm">
            <input v-model="form.nation" placeholder="nation" class="field-input" />
            <span class="field-line"></span>
          </div>
        </div>
        <div class="form-row">
        ...
        <transition name="toast">
          <div v-if="msg.text" class="toast" :class="'toast--' + msg.type">
            {{ msg.text }}
          </div>
        </transition>

        <!-- 提交按钮 -->
        <button class="submit-btn" @click="handleCreate" :disabled="submitting">
          <template v-if="!submitting">SUBMIT</template>
...
const form = ref({ name: '', time: '', type: '', director: '', actor: '', nation: '' })
...
async function handleCreate() {
  ...
  msg.text = ''
  try {
    const result = await movieService.addMovie(form.value)
    msg.text = '新增成功，ID：' + result
    msg.type = 'success'
    setTimeout(() => { router.push(`/movie/${result}`) }, 800)
  } catch {
    msg.text = '新增失败，请重试'
    msg.type = 'fail'
  } finally {
...
.page {
  ...
  --rose:         #C97D7D;
  --rose-soft:    rgba(201,125,125,0.25);
  --cream:        #EDE4DB;
...
.toast--fail {
  ...
  border: 1px solid rgba(201,125,125,0.2);
  color: #C97D7D;
}
.toast--warn {
